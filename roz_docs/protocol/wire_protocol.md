...
- **Controller**: Execute its configured disconnect stored procedure (MCU-R25). Continue indicating status via LED.
- **Base station**: Mark the controller as disconnected and cease sending commands.

On reconnection, the full version handshake (Section 5) is repeated. After a successful handshake, the controller pushes its full state (MCU-R26) via ActuatorTelemetry, SensorTelemetry, SystemTelemetry, and PostResult messages.
