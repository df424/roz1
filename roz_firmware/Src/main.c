...
/* USER CODE BEGIN PV */
static pal_transport_t transport;
static framing_state_t framing;
...
int main(void)
{
  ...
  pal_gpio_init();
  pal_pwm_init();
  pal_transport_init(&transport);
  pal_capture_init();

  /* Framing and protocol */
  framing_init(&framing, &transport);
  protocol_init(&framing);
  ...
  clock_sync_init();
  telemetry_init();
  system_init();              /* enters SYS_POST */

  /* POST + homing */
  actuator_homing_start();
  while (!actuator_homing_complete()) {
      actuator_manager_tick(pal_tick_ms());
  }

  /* Transition to handshake */
  system_set_state(SYS_HANDSHAKE);
  handshake_init();
  handshake_start();

  /* Wait for handshake to complete */
  while (handshake_get_state() != HS_COMPLETE &&
         handshake_get_state() != HS_FAILED) {
      if (framing_poll(&framing, &rx_frame)) {
          system_on_frame_received();
          protocol_dispatch(&rx_frame);
      }
      system_tick(pal_tick_ms());
  }

  if (handshake_get_state() == HS_COMPLETE) {
      system_set_state(SYS_RUNNING);
